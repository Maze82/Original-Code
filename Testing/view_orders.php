...
$query1 .=" group by O.id order by O.ordered_date desc";	
 ...
 $select="select  sum(product_quantity) as total_items,sum(total_product_weight) as p_weight  from order_products where order_id=$id";
        ...
        $total_weight= number_format($rec['p_weight'],2);
        ...
        $total_filled_weight=number_format($rec1['filled_weight'],2);
...
function GetOrderProducts($db)
{
    ...
    $select="select id,product_name from products order by product_name asc";	
	...
	if($db->num_rows())
	{
		while($row=$db->fetch_assoc())
  		{  
            ...
            $product_LIST .="<option value=\"$row[id]\">$row[product_name]</option>";
  		}
...
function GetOrderWards($db)
{
	...
	$select="select id,ward_name from wards order by ward_name asc";
	$db->query($select);
	if($db->num_rows())
	{
		while($row=$db->fetch_assoc())
