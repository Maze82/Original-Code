...
{
        global $id,$ordered_date,$date_filled,$ward_id,$ordering_person,$email,$bag_delivered,$cage_delivered,$total_weight,$item_ordered,$status_unfill,$status_fill,$products,$status,$WARD_LIST,$current_date,$item_order,$delivery_date,$counter,$total_weight,$filled_ordered,$filled_weight,$comments;
        ...
        if($db->num_rows() > 0)
        {
             ...
             //$WARD_LIST             =   getWardlist($db1,$rec['ward_id']);
              ...
              //Matt has added this line of code down
              $bag_delivered        =   $rec['bag_delivered'];
              $cage_delivered        =   $rec['cage_delivered'];
              //up from here
 ...
 $select1="select OP.*,O.item_ordered from order_products as OP left join orders as O on O.id=OP.order_id
	 ...
	 $total_weight_val=0;
	  ...
	  {
	  		$products .="<tr>
                                <th align=\"right\"width:120px\">Product</th>
                                <th align=\"right\"width:40px\">Ordered</th>
                                <th align=\"right\"width:40px\">Dispatched</th>
                            </tr>";
			...
			while($row=$db->fetch_assoc()){
				...
				//$total_weight_val=
				$products .="<tr>
                                <td align=\"right\">$product_name<input type=\"hidden\" name=\"pid$product_id-$i\"style=\"width:120px\"value=\"$product_id\" ></td>
...
function update($db,$db1)
{
 global  $SITE_URL,$MEMBER_ID;
	  ...
	  /* $date_parts1=explode("/", $date_filled);
       ...
       $bag_delivered       =   $_POST['bag_delivered'];
       $cage_delivered      =   $_POST['cage_delivered'];


		if($status=='FILLED')
		 { //$date_filled= $current_date;
		  $date_filled          =   strtotime(str_replace('/', '-',$date_filled));
		  //$date_filled          =   strtotime(str_replace('/', '-',$_POST['date_filled']));
		 }
		 else
		 {
       ...
       $query="update orders set
                ...
                bag_delivered='$bag_delivered',
                cage_delivered='$cage_delivered',
...
$username = "laundryk_erm";
$password = 'iZc,~oMFO1pm';
$hostname = "localhost";
...
$selected = mysql_select_db("laundryk_erm",$dbhandle)
...
$selected = mysql_select_db("fm",$dbhandle)
  or die("Could not select database");*/
    ...
    $to_email="sales@laundrykingtas.com.au";
