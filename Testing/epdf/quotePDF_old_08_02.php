   ...
   $sql2="select count(P.id) as total_filled_item,sum(P.product_weight) as total_filled_weight from order_products as P 
	...
	if($db->num_rows())
	{
		while($rec2=$db->fetch_assoc()){
			...
			$sql5="select count(P.id) as tot_filled_item,sum(product_weight) as tot_filled_weight from order_products as P
			   	left join orders as O on O.id=P.order_id where O.ward_id='$wardid' and order_id in ($id) and O.status='FILLED'";
		  ...
		  $sql6="select product_id,product_name,product_weight from order_products where order_id in ($id) group by product_id";
			...
			if($db1->num_rows())
			{
				...
				$sql8A="select count(P.id) as tot_pro_unfilled_items,sum(product_weight) as tot_pro_unfilled_product_weight from order_products as P
			   	left join orders as O on O.id=P.order_id where product_id='$prid' and order_id in ($id) and O.status='UNFILLED'";
				
				$db3->query($sql8A);
				if($db3->num_rows())
				{
					$rec6A=$db3->fetch_assoc();
					
					$tot_pro_unfilled_product_weight=$rec6A['tot_pro_unfilled_product_weight'];
					$sum_product_unfilled_weight += $tot_pro_unfilled_product_weight;

					
					$tot_pro_unfilled_items =$rec6A['tot_pro_unfilled_items'];
					$sum_product_unfilled +=$tot_pro_unfilled_items;
				}

				$unfilled_pro_per=($tot_pro_unfilled_items/$tot_pro_items)*100;
				...
				$product_report .="<tr>
							...
							<td width=\"3%\">&nbsp;</td>
							<td width=\"10%\" style=\"font-size:14px;text-align:center\">$tot_pro_items</td>
							<td width=\"3%\">&nbsp;</td>
							<td width=\"10\" style=\"font-size:14px;text-align:center\">$tot_pro_filled_items</td>
							<td width=\"5%\">&nbsp;</td>
							<td width=\"10\" style=\"font-size:14px;text-align:center\">".number_format($filled_pro_per,2)." %</td>
							<td width=\"3%\">&nbsp;</td>
							<td width=\"10%\" style=\"font-size:14px;text-align:center\">$tot_pro_unfilled_product_weight kg</td>
							<td width=\"3%\">&nbsp;</td>
							<td width=\"10\" style=\"font-size:14px;text-align:center\">$tot_pro_unfilled_items</td>
							<td width=\"3%\">&nbsp;</td>
							<td width=\"10\" style=\"font-size:14px;text-align:center\">".number_format($unfilled_pro_per,2)." %</td>
							<td width=\"3%\">&nbsp;</td>
							<td width=\"10%\" style=\"font-size:14px;text-align:center\">$tot_pro_unfilled_product_weight kg</td>
						</tr>";
