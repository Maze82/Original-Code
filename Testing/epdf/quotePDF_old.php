...
// $orderdate12=mktime(0,0,0,$month,$day,$year);
 ...
 $select="select  sum(product_quantity) as total_items,sum(total_product_weight) as p_weight  from order_products where order_id=$id";
		...
		//print_r($array_orderId);
		foreach ($array_orderId as $valorderId) 
		{
			$orderArr .=$valorderId.',';
		}
		$orderQryArr  =rtrim($orderArr,',');
	/*	foreach ($array_orderId as $valorderId) 
		{*/
			echo $select1="SELECT OP. * , O.item_ordered,sum(OP.product_quantity)as product_quantity, SUM( OP.dispatched_quantity ) AS dispatched_quantity11,  COUNT( OP.dispatched_quantity ) AS qty FROM order_products AS OP LEFT JOIN orders AS O ON O.id = OP.order_id WHERE OP.order_id IN  ($orderQryArr) GROUP BY OP.product_id"; 
			//die();
			$total_weight_val=0;
			$db->query($select1);
			if($db->num_rows())
			{
				...
				$dispatched_quantity11 = $row['dispatched_quantity11'];
				$item_order            = $row['item_ordered'];
				$product_weight        = $row['product_weight'];
				//$product_quantity_unique_item= $product_quantity * $qty;
			
				$filled_weight   = $dispatched_quantity11 * $product_weight;
				//$unfilled_item   = $product_quantity-$dispatched_quantity;
				$unfilled_item   = $product_quantity-$dispatched_quantity11;
				
				$unfilled_weight = $product_weight * $unfilled_item;
	
				$total_filled_item+= $total_filled_item + $dispatched_quantity11;
				$sum_weight += $product_weight;
				
				//$sum_product_items +=$product_quantity;
				$sum_product_items +=$product_quantity;
				$sum_product_filled +=$dispatched_quantity11;
				
	        	//$filled_pro_per1      = ($dispatched_quantity/$product_quantity)*100;
				$filled_pro_per1      = ($dispatched_quantity11/$product_quantity)*100;
				//$unfilled_pro_per1    = ($unfilled_item/$product_quantity)*100;
				$unfilled_pro_per1    = ($unfilled_item/$product_quantity)*100;
				$sum_product_filled_weight+=$filled_weight;
				$sum_product_unfilled+=$unfilled_item;
				$sum_product_unfilled_weight+=$unfilled_weight ;
				...
				$product_report .="<tr>
				            ...
				            <td width=\"26%\" style=\"font-size:14px;text-align:left\">$product_name</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9%\" style=\"font-size:14px;text-align:center\">$product_weight</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9%\" style=\"font-size:14px;text-align:center\">$product_quantity</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9\" style=\"font-size:14px;text-align:center\">$dispatched_quantity11</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9\" style=\"font-size:14px;text-align:center\">".number_format($filled_pro_per1,2)." %</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9%\" style=\"font-size:14px;text-align:center\">$filled_weight kg</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9\" style=\"font-size:14px;text-align:center\">$unfilled_item</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9\" style=\"font-size:14px;text-align:center\">".number_format($unfilled_pro_per1,2)." %</td>
							<td width=\"2%\">&nbsp;</td>
							<td width=\"9%\" style=\"font-size:14px;text-align:center\">$unfilled_weight kg</td>
						</tr>";
